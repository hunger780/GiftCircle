...
const App: React.FC = () => {
  ...
  const myActiveEvents = useMemo(() => {
    ...
    const visibleEvents = combined.filter(e => !me.hiddenEventIds?.includes(e.id));

    return Array.from(new Map(visibleEvents.map(item => [item.id, item])).values());
                      ...
                      >
                          ...
                          {e.userId !== me.id && (
                              <button 
                                ...
                                className="absolute top-2 right-2 p-1.5 bg-black/20 hover:bg-black/40 rounded-full text-white/80 hover:text-white