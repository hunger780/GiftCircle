...
const App: React.FC = () => {
  ...
  const [suggestedContributionAmount, setSuggestedContributionAmount] = useState<number | null>(null);
  ...
  const handleContribute = (amount: number, type: ContributionType, isAnonymous: boolean, isAmountHidden: boolean) => {
    ...
    setSuggestedContributionAmount(null);
  };

  const handleStartContribution = (item: WishlistItem, amount?: number) => {
    setContributingItem(item);
    if (amount) setSuggestedContributionAmount(amount);
    else setSuggestedContributionAmount(null);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans max-w-screen-md mx-auto shadow-2xl">
      ...
      <main className="pb-24 pt-4">
        {view === 'HOME' && <HomeView events={myActiveEvents} wishlist={displayedWishlist} wishlistTab={wishlistTab} setWishlistTab={setWishlistTab} setView={setView} handleEventClick={(id) => { setViewingEventId(id); setView('EVENT_DETAIL'); }} handleHideEvent={(e, id) => { e.stopPropagation(); setCurrentUserData({ ...me, hiddenEventIds: [...(me.hiddenEventIds || []), id] }) }} handleContribute={handleStartContribution} handleCancelItem={handleCancelItem} onItemClick={setViewingItemId} me={me} currencySymbol={currencySymbol} />}
        ...
        {view === 'PROFILE' && selectedFriendId && <FriendProfileView friendId={selectedFriendId} events={events} wishlist={displayedWishlist} currencySymbol={currencySymbol} onBack={() => setView('FRIENDS')} onItemClick={setViewingItemId} onContribute={handleStartContribution} />}
        ...
        {view === 'GIFT_FLOW' && <GiftFlowView onBack={() => setView('HOME')} friends={friends} wishlist={wishlist} currencySymbol={currencySymbol} onContribute={handleStartContribution} onItemClick={setViewingItemId} />}
      </main>
      ...
      {contributingItem && <ContributionModal item={contributingItem} defaultAmount={suggestedContributionAmount || me.settings.defaultGiftAmount} maxAmount={me.settings.maxGiftAmount} currencySymbol={currencySymbol} onClose={() => setContributingItem(null)} onContribute={handleContribute} />}
